...
impl WorldMeta {
    ...
    fn decode(bytes: &[u8]) -> Result<WorldMeta, FrameError> {
        ...
        String::from_utf8(bytes[8..].to_vec())
            ...
            .map_err(|_| FrameError::ChecksumMismatch {
                expected: 0,
                actual: 0,
            })
    }
...
mod tests {
    ...
    fn p3d102_patch_round_trips_and_frames_exactly() {
        ...
        assert_eq!(
            raw,
            crate::framing::frame(&FormatHeader::current(), b"terrain-bytes")
        );
    ...
    fn p3d102_disk_loads_refuse_foreign_wrong_and_corrupt() {
        ...
        let foreign_path = world_root(root, "beta").join(patch_rel_path(coord));
        ...
        assert_eq!(
            load_world_meta(root, "beta", &SUP).expect("meta load"),
            meta
        );
    }
    ...
    fn p3d102_tmp_residue_is_invisible_and_saves_replace() {
        ...
        assert_eq!(
            load_patch(root, "gamma", coord, &SUP).expect("load"),
            b"v2-longer"
        );
